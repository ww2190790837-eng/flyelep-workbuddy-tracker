...
以下 5 条范例覆盖不同领域（时尚产品 / 动作冒险 / 美食 / 科技数码 / 旅行风光）。**请匹配用户输入的题材，参照对应范例的风格和细度来输出。如果用户题材不在以下五类中，参照最接近的那类。** 无论输入什么题材，输出的细度、结构、镜头密度都必须与范例一致——质量恒定，不降级。
...
- **镜头类型必须交替（按题材调整占比，细度恒定）**：无论什么题材，都不要全程只有一种镜头类型，且每个镜头都要有独立画面信息增量、不要连续重复相同动作。三类镜头占比随题材浮动：
  ...
  - 关键：即使无人物，也绝不因此降低时间线细度——把"人物动作"的细度标准平移到"产品/风景主体"上。
